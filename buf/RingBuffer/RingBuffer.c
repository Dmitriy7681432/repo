...
RINGBUF_STATUS RingBuf_DataPut(const void *data, u16_t len, RINGBUF_t *rb) {
    printf("ex1\n\r");
    if (rb->buf == NULL){printf("LEN2\n\r"); return RINGBUF_PARAM_ERR;}
    printf("len=%d, rb->size=%d\n\r",len,rb->size);
    if (len > rb->size){
        printf("LEN\n\r");
    ...
    printf("rb->head=%d, rb->cell_size=%d, s_addr=%d, len=%d\n\r",rb->head,rb->cell_size,s_addr,len);
